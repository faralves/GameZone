...
namespace GameZone.Core.Utils
{
    public static class StringUtils
    {
        ...
        public static Boolean ComparaPropriedades(Object novoObjeto, Object velhoObjeto, string[] CamposAverificar)
        {

            Type newType = novoObjeto.GetType();
            Type oldType = velhoObjeto.GetType();

            if (newType == oldType)
            {

                PropertyInfo[] propriedades = newType.GetProperties();

                foreach (PropertyInfo pi in propriedades)
                {
                    try
                    {
                        if (pi.PropertyType.Name != "Proprietario" && pi.PropertyType.Name != "Motor" && pi.PropertyType.Name != "Gravame" &&
                            pi.PropertyType.Name != "Debito" && pi.PropertyType.Name != "Restricao" && pi.PropertyType.Name != "ComunicacaoVenda" &&
                            pi.PropertyType.Name != "BloqueioRenajud" && pi.PropertyType.Name != "ConsPontuacao" && pi.PropertyType.Name != "Veiculo" &&
                            pi.PropertyType.Name != "Ficha" && pi.PropertyType.Name != "BloqueioDetran" && pi.PropertyType.Namespace != "System.Collections.Generic")
                        {
                            string newValue = newType.GetProperty(pi.Name).GetValue(novoObjeto, null) == null ? "" : newType.GetProperty(pi.Name).GetValue(novoObjeto, null).ToString();
                            string oldValue = oldType.GetProperty(pi.Name).GetValue(velhoObjeto, null) == null ? String.Empty : oldType.GetProperty(pi.Name).GetValue(velhoObjeto, null).ToString();


                            if (CamposAverificar.Contains(pi.Name))
                            {
                                if (newValue != oldValue)
                                {
                                    ...
                                    {
                                        case ("System.Int16"):
                                            Int16 valInt16 = Int16.Parse(newValue);
                                            if (valInt16 > 0)
                                            {
                                                return true;
                                            }
                                            break;
                                        case ("System.Int32"):
                                            Int32 valInt32 = Int32.Parse(newValue);
                                            if (valInt32 > 0)
                                            {
                                                return true;
                                            }
                                            break;
                                        case ("System.Int64"):
                                            Int64 valInt64 = Int64.Parse(newValue);
                                            if (valInt64 > 0)
                                            {
                                                return true;
                                            }
                                            break;
                                        case ("System.Decimal"):
                                            Decimal valor = Decimal.Parse(newValue);
                                            if (valor > 0)
                                            {
                                                return true;
                                            }
                                        ...
                                        case ("System.DateTime"):
                                            DateTime data = DateTime.Parse(newValue);
                                        ...
                                        case ("System.String"):
                                            Int32 numNew = 0;
                                            Boolean newValido = Int32.TryParse(newValue, out numNew);

                                            if (newValido)
                                            {
                                                Int32 numOld = 0;
                                                Boolean oldValido = Int32.TryParse(oldValue, out numOld);
                                                if (numNew > 0)
                                                {
                                                    return true;
                                                }
                                            ...
                                            else
                                            {
                                                ...
                                                if (!IsNullOrWhiteSpaces(newValue))
                                                {
                                                    return true;
                                                }
                                            }
